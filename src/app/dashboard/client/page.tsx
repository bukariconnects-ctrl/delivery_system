...
export default function ClientDashboard() {
  ...
  const openMenu = async (rest: Restaurant) => {
    ...
    if (data) {
      setMenuItems(data);
      // Resolve signed image URLs via DFS (Req 10 — client-side cache)
    ...
    <DashboardShell role="client" title="لوحة تحكم العميل">
      ...
      {view === "menu" && selectedRestaurant && (
        <div dir="rtl">
          ...
          <div className="relative mb-8 overflow-hidden rounded-2xl bg-gradient-to-br from-orange-500 via-amber-500 to-yellow-400 p-6 text-white shadow-lg">
              ...
              <div>
                <div className="mb-2 flex items-center gap-2">
                  ...
                  <span className="rounded-full bg-white/20 px-2.5 py-0.5 text-xs font-bold backdrop-blur-sm">
                    {selectedRestaurant.cuisine_type || "مطعم"}
                  </span>
                  <span className="flex items-center gap-1 rounded-full bg-green-400/30 px-2.5 py-0.5 text-xs font-bold backdrop-blur-sm">
                    <Star size={10} className="fill-white" /> 4.8
                  </span>
                </div>
                <h2 className="text-2xl font-bold">{selectedRestaurant.name}</h2>
                {selectedRestaurant.description && (
                  <p className="mt-1 max-w-md text-sm opacity-90">{selectedRestaurant.description}</p>
                )}
              </div>
              <button
                onClick={() => { setView("restaurants"); setCart([]); }}
                className="flex items-center gap-1 rounded-xl bg-white/20 px-4 py-2 text-sm font-medium backdrop-blur-sm transition hover:bg-white/30"
              >
          ...
          <div className="mb-5 flex items-center justify-between">
            <h3 className="text-lg font-bold text-gray-800 dark:text-gray-100">القائمة</h3>
            <span className="text-sm text-gray-500">{menuItems.length} وجبة متاحة</span>
          </div>

          {loading ? (
            <div className="grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-3">
              {Array.from({ length: 6 }).map((_, i) => (
                <div key={i} className="rounded-2xl bg-white p-0 shadow-sm dark:bg-gray-800">
                  <div className="aspect-[4/3] animate-pulse rounded-t-2xl bg-gray-200 dark:bg-gray-700" />
